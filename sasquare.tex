...
\begin{document}
    ...
    \end{figure}
    \newpage
    To enumerate the cell indices within the diagonal, we shall use the intersection function $\operatorname{i}$ (see Chapter \ref{intersection}) where $r = c$. We shall name the set $D$. For this example, $b = 5$ and $n$ is the nth cell of the diagonal. To simplify, since $r = c$, we will replace $r$ and $c$ with $n$. 
    \begin{equation}
        \begin{split}
          \operatorname{i}(b,r,c) &= b(r-1)+(c-1) \\
                &= b(n-1)+(n-1) \\
                &= bn-b+n-1 \\
            D &= \{ bn-b+n-1 : n = 1, \dots, b) \} \\
            \\
            \operatorname{D}(5,1) &= (5)(1) - 5 + (1) - 1 =  0 \\
            \operatorname{D}(5,2) &= (5)(2) - 5 + (2) - 1 =  6 \\
            \operatorname{D}(5,3) &= (5)(3) - 5 + (3) - 1 =  12 \\
            \operatorname{D}(5,4) &= (5)(4) - 5 + (4) - 1 =  18 \\
            \operatorname{D}(5,5) &= (5)(5) - 5 + (5) - 1 =  24 \\
            D &= \{0, 6, 12, 18, 24 \} \\
        \end{split}
    \end{equation}
    ...
    To enumerate the cell indices within the anti-diagonal, we shall use the function $\operatorname{A}$ for finding the nth cell of the anti-diagonal. We get the function $\operatorname{A}$ using the $\operatorname{i}$ function (see Chapter \ref{intersection}) where $r = b - c + 1$, then we replace $c$ with $n$ For this example, we will find the antidiagonal cell indices in a 5-square.
    \begin{equation}
        \begin{split}
            \operatorname{i}(b,r,c) &= b(r-1)+(c-1) \\
                &= b((b - c + 1) - 1) + (c-1) \\
