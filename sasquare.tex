...
\begin{document}
    ...
    The Anti-offdiagonal cells are the cells not found in the anti-diagonal. Like the off-diagonal cells, there are two kinds of anti-offdiagonal cells: the anti-superdiagonal and the anti-subdiagonal cells. The anti-superdiagonal cells are above and left of the anti-diagonal whilst the anti-subdiagonal cells are below and right of the anti-diagonal.
    ...
    We shall call the index of a descending slant a descending index or $\kappa$ (kappa) index. To determine the $\kappa$ index of the cell, we shall use function $\kappa$.
    \begin{equation}
        \kappa(b,n) = b + \delta(b,n)
    \end{equation}

    We shall call the index of an ascending slant an ascending index or $\upsilon$ (upsilon) index. To determine the $\upsilon$ index of the cell, we shall use function $\upsilon$.
    \begin{equation}
    ...
    \subsection{Multiples of the Base Until Its Square}
    ...
    \begin{equation}
        u(b,x) = b^{2 + \left\lceil -\frac{x}{b} \right\rceil }|b-x|
    \end{equation}
    \begin{equation}
    ...
    Function $v$ is a function that either yields a multiple of $b$ or subtracts $x$ from $b$ depending on whether $x$ exceeds or fall short of $b$.
    \begin{equation}
        v(b,x) = |b-x|(b-1) + b^{\left\lceil \frac{x}{b} \right\rceil}(b-x)
    \end{equation}
    \begin{equation}
        V = \{ v(b,x) : x \in \mathbb{N}, 0 \leq x \leq 2b \}
    \end{equation}
    The range of set $V$ has these own traits:
    \begin{itemize}
        \item When $x = 2b$ then $v(b,x) = -b$
        \item When $b \leq x \leq 2b$ then $v(b,x) = b - x$
    \end{itemize}
